...
class Application(tk.Frame):
	...
	def title_frame(self):
		self.title = tk.Label(self, bg='#56ff93',font=("Helvetica", 14), anchor='w')
		self.title.configure(width=35, height=3)
		self.title['text'] = '\t Folder Locker / Unlocker'
		self.title.grid(row=0, column=0, columnspan=2)

	def body_frame(self):
		self.body = tk.Frame(self, width=380, height=300, bg="#fff87d")
		self.body.grid(row=1, column=0, columnspan=2, pady=10)

	def status_frame(self):
		self.status = tk.Text(self, width=50, height=3, fg='black', bg="#fff87d")
	...
	def main_frame(self):
		...
		self.lock_button = tk.Button(self.body, font =('Verdana', 15),bg="#64a6ff", command=self.lock_frame)
		...
		self.unlock_button = tk.Button(self.body, font =('Verdana', 15), bg="#64a6ff", command=self.unlock_frame)
	...
	def unlock_frame(self):
		...
		self.unlock =tk.Button(self.body, bg='#64a6ff', width=20)
...
if __name__ == '__main__':
	root = tk.Tk()
	root.configure(bg='#56ff93')
	root.geometry('400x305')
	root.title('Folder Locker/Unlocker')
	root.resizable(0,0)
	lock_icon = PhotoImage(file=sys._MEIPASS+'\\lock.png').subsample(2,2)
	unlock_icon = PhotoImage(file=sys._MEIPASS+'\\unlock.png').subsample(2,2)
	back_icon = PhotoImage(file=sys._MEIPASS+'\\back.png')
	choose_folder_icon = PhotoImage(file=sys._MEIPASS+'\\choose_folder.png')
